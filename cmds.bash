...
if [[ $chan == $botnick ]] ; then
    ...
    if `echo $saying | grep -Fi '\broll\b' > /dev/null` ; then
        output=`echo $nick $chan $saying | ./roll.bash`
        echo "$output"
    elif `echo $saying | grep -Fi '\bhelp\b' > /dev/null` ; then # Help docs
        echo "NOTIFY ${nick//:} :Please visit http://web.cecs.pdx.edu/~nickg/dicebothelp.txt for help using dicebot"
    elif `echo $saying | grep -Fi '\bstats\b' > /dev/null` ; then # display stats
...
# Regular channel commands
    ...
    if `echo $saying | grep -Fi $regex > /dev/null` ; then
        if `echo $saying | grep -Fi '\bhelp\b' > /dev/null` ; then # HELP
            echo "PRIVMSG $chan $nick: Please visit http://web.cecs.pdx.edu/~nickg/dicebothelp.txt for help using dicebot"
        elif `echo $saying | grep -Fi '\broll\b' > /dev/null` ; then # ROLL
            output=`echo $nick $chan $saying | ./roll.bash`
            echo "$output" 		
        elif `echo $saying | grep -Fi '\bwho\b' > /dev/null` ; then # WHO
        ...
        elif `echo $saying | grep -Fi '\bcommands\b' > /dev/null` ; then # COMMANDS
            echo "PRIVMSG $chan $nick: Dicebot Commands: commands, help, !roll, roll, stats, who"
        elif `echo $saying | grep -Fi '\bstats\b' > /dev/null` ; then # STATS
        ...
        elif `echo $saying | grep -Fi '\bsource\b' > /dev/null` ; then # SOURCE code on GITHUB
            echo "PRIVMSG $chan $nick: Dicebot source code: https://github.com/gilben1/dicebot"
        fi
    elif `echo $saying | grep -Fi '\!roll\b' > /dev/null` ; then # ROLL AGAIN
        output=`echo $nick $chan $saying | ./roll.bash`
        echo "$output"
    fi
fi
